...
class SwiftDexService: ObservableObject {    
    ...
    func pokemonDexNumbers(matching text: String?) -> Results<PokemonDexNumber> {
        ...
        return selectedPokedex.pokemonSpeciesDexNumbers.filter("ANY species.names.name CONTAINS [c] '\(text)' OR ANY species.pokemon.forms.names.pokemonName CONTAINS [c] '\(text)'").sorted(byKeyPath: "pokedexNumber")
    }
