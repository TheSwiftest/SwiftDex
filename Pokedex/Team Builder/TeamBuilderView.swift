...
struct TeamBuilderView: View {
    ...
    @State private var selectedCategory: ShowdownCategory? = nil
    ...
    var body: some View {
        GeometryReader { fullViewProxy in
            ZStack {
                VStack(spacing: 0) {
                    TeamBuilderHeaderView(searchText: $searchText, selectedTeam: $selectedTeam, selectedCategory: $selectedCategory, showFormatSelectionsView: $showFormatSelectionsView, selectedFormatSourceFrame: $formatSelectionSourceFrame, selectedGeneration: $selectedGeneration, showGenerationSelectionsView: $showGenerationSelectionsView, selectedGenerationSourceFrame: $generationSelectionSourceFrame).environmentObject(swiftDexService).environmentObject(pokemonShowdownService)
                    ...
                    if pokemonShowdownService.teams.count > 0 {
                        ...
                        VStack {
                            Spacer()
                            Text("It looks like you have no teams yet! You can start fresh by creating a new one\n\nOr if you're a Pokémon pro feel free to import a backup from Pokémon Showdown! Just make sure it's copied to your clipboard.")
                                .font(.headline)
                ...
                GeometryReader { formatSelectionsProxy in
                    CategorySelectionsView(categories: swiftDexService.showdownCategories(for: selectedGeneration).compactMap{$0}, showView: $showFormatSelectionsView, selectedCategory: $selectedCategory, sourceFrame: $formatSelectionSourceFrame, searchText: $searchText)
                }
                .edgesIgnoringSafeArea(.top)
                
                GeometryReader { generationSelectionsView in
                    GenerationSelectionsView(generations: swiftDexService.generations, showView: $showGenerationSelectionsView, selectedGeneration: $selectedGeneration, sourceFrame: $generationSelectionSourceFrame, searchText: $searchText)
                }
                .edgesIgnoringSafeArea(.top)
            }
        }
