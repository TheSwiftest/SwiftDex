...
struct BattleSimulatorView: View {
    ...
    private var maxDamage: Int {
        ...
        if (auroraVeilActive || (lightScreenActive && move.damageClass?.identifier == "special") || (reflectActive && move.damageClass?.identifier == "physical")) && !criticalHit && attackingPokemon.ability?.identifier != "infiltrator" && defendingPokemon.ability?.identifier != "screen-cleaner" {
            modifier *= 0.5
        }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 30) {
                ...
                BattleSimDamageView(maxDamage: maxDamage, minDamage: minDamage, avgDamage: avgDamage, attackingPokemon: attackingPokemon, defendingPokemon: defendingPokemon, selectedMove: selectedMove)
                
                BattleSimWeatherView(weather: $weather)
                
                BattleSimModifiersView(defenderAtFullHealth: $defenderAtFullHealth, wonderRoom: $wonderRoom, criticalHit: $criticalHit, attackerBurned: $attackerBurned, defenderMinimized: $defenderMinimized, defenderDigging: $defenderUsingDig, defenderDiving: $defenderUsingDive, reflectActive: $reflectActive, lightScreenActive: $lightScreenActive, auroraVeilActive: $auroraVeilActive, allyHasFriendGuard: $allyHasFriendGuard)
            }
            .padding()
        }
    }
